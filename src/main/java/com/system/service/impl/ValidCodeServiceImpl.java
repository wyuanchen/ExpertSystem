...
public class ValidCodeServiceImpl implements ValidCodeService{
    private static final String keyValidCode="validCode";
    private ValidCodeGenerator validCodeGenerator=new ValidCodeGenerator();
    ...
    public String generateValidCode(HttpServletRequest request,HttpServletResponse response){
        ...
        saveValidCodeToSession(validCode,request.getSession());
        return validCode;
    }

    /**
     * 检验验证码是否正确
     * @param validCode 待检测的验证码
     * @param request
    ...
    private void saveValidCodeToSession(String validCode,HttpSession session){
        session.setAttribute(keyValidCode,validCode);
    }
    private String getValidCodeFromSession(HttpSession session){
        return (String) session.getAttribute(keyValidCode);
    }
}
